...
def parse_args(args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="ogbl-biokg",
        help="dataset name, default to biokg",
    )
    ...
    parser.add_argument(
        "--print_on_screen", action="store_true", help="log on screen or not"
    )
    parser.add_argument(
        "--ntriples_eval_train",
        type=int,
...
def main(args):
    ...
    for i in tqdm(range(len(train_triples["head"]))):
        ...
        head_type, tail_type = (
            train_triples["head_type"][i],
            train_triples["tail_type"][i],
        )
        ...
        for step in range(init_step, args.max_steps):
            ...
            if args.do_valid and step % args.valid_steps == 0 and step > 0:
                ...
                metrics = kge_model.test_step(
                    kge_model, valid_triples, args, entity_dict
                )
    ...
    if args.evaluate_train:
        ...
        metrics = kge_model.test_step(
            kge_model, small_train_triples, args, entity_dict, random_sampling=True
        )
        log_metrics("Train", step, metrics, writer)
