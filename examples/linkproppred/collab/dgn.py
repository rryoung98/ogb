    ...
    def __init__(
        ...
        if self.type_norm == "batch":
            ...
            self.bn = torch.nn.BatchNorm1d(
                hidden_features * self.num_groups, momentum=0.3
            )
            self.group_func = torch.nn.Linear(
                hidden_features, self.num_groups, bias=True
            )
        else:
            pass
    ...
    def forward(self, x):
        if self.type_norm == "None":
            ...
            if self.num_groups == 1:
                ...
                x_temp = torch.cat(
                    [
                        score_cluster[:, group].unsqueeze(dim=1) * x
                ...
                x_temp = (
                    self.bn(x_temp)
                    .view(-1, self.num_groups, self.hidden_features)
                    .sum(dim=1)
                )
