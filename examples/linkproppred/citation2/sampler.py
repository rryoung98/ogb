...
class PositiveLinkNeighborSampler(NeighborSampler):
    def __init__(self, edge_index, sizes, num_nodes=None, **kwargs):
        ...
        super(PositiveLinkNeighborSampler, self).__init__(
            edge_index, sizes, edge_idx, num_nodes, **kwargs
        )
...
class NegativeLinkNeighborSampler(NeighborSampler):
    def __init__(self, edge_index, sizes, num_nodes=None, **kwargs):
        edge_idx = torch.arange(edge_index.size(1))
        super(NegativeLinkNeighborSampler, self).__init__(
            edge_index, sizes, edge_idx, num_nodes, **kwargs
        )
...
@torch.no_grad()
def test(
    model, predictor, x, subgraph_loader, split_edge, evaluator, batch_size, device
):
    predictor.eval()
...
def main():
    ...
    pos_loader = PositiveLinkNeighborSampler(
        edge_index,
        sizes=[15, 10, 5],
    ...
    neg_loader = NegativeLinkNeighborSampler(
        edge_index,
        sizes=[15, 10, 5],
        batch_size=args.batch_size,
        shuffle=False,
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            loss = train(model, predictor, x, pos_loader, neg_loader, optimizer, device)
