...
class MAG240M(LightningDataModule):
    ...
    def prepare_data(self):
        ...
        if not osp.exists(path):  # Will take approximately 16 minutes...
            ...
            edge_types = [
                torch.full(x.size(), i, dtype=torch.int8) for i, x in enumerate(rows)
            ]
            ...
            full_adj_t = SparseTensor(
                row=row, col=col, value=edge_type, sparse_sizes=(N, N), is_sorted=True
            )
        ...
        if not osp.exists(done_flag_path):  # Will take ~3 hours...
            ...
            x = np.memmap(
                path, dtype=np.float16, mode="w+", shape=(N, self.num_features)
            )
            ...
            for _ in range(num_layers - 1):
                self.convs.append(
                    ModuleList(
                        [
                            SAGEConv(
                                hidden_channels, hidden_channels, root_weight=False
                            )
                            for _ in range(num_relations)
                        ]
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", type=str, default="rgat", choices=["rgat", "rgraphsage"]
    )
