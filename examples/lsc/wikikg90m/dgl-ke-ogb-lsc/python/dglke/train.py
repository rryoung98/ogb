...
def main():
    ...
    train_data = TrainDataset(
        dataset, args, ranks=args.num_proc, has_importance=args.has_edge_importance
    )
    ...
    if args.num_proc > 1:
        ...
        for i in range(args.num_proc):
            ...
            print(
                "Training sampler for proc %d created, it takes %s seconds"
                % (i, time.time() - t1)
            )
    ...
    if args.test:
        if args.num_test_proc > 1:
            ...
            for i in range(args.num_test_proc):
                ...
                print(
                    "Test sampler for proc %d created, it takes %s seconds"
                    % (i, time.time() - t1)
                )
        else:
            # test_sampler_head = eval_dataset.create_sampler('test', args.batch_size_eval,
