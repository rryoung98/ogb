...
class KGDatasetUDDRaw(KGDataset):
    ...
    * test stores the triples in the test set. In format [src_name, rel_name, dst_name]
    ...
    def __init__(self, path, name, delimiter, files, format, has_edge_importance=False):
        ...
        assert len(files) == 1 or len(files) == 3, (
            "raw_udd_{htr} format requires 1 or 3 input files. "
    ...
    def read_triple(self, path, mode, skip_first_line=False, format=[0, 1, 2]):
        ...
        if self.has_edge_importance:
            ...
            assert np.min(e_impts) > 0.0, "Edge importance score should > 0"
...
class KGDatasetUDD(KGDataset):
    ...
    * test stores the triples in the test set. In format [src_id, rel_id, dst_id]
    ...
    def __init__(self, path, name, delimiter, files, format, has_edge_importance=False):
        ...
        format = _parse_srd_format(format)
        assert len(files) == 3 or len(files) == 5, (
            "udd_{htr} format requires 3 or 5 input files. "
    ...
    def read_triple(self, path, mode, skip_first_line=False, format=[0, 1, 2]):
        ...
        with open(path) as f:
            ...
            for line in f:
                ...
                try:
                    ...
                    print(
                        "For User Defined Dataset, both node ids and relation ids in the "
        ...
        print("Finished. Read {} {} triples.".format(len(heads), mode))
        assert (
            np.max(heads) < self.n_entities
        ), "Head node ID should not exceeds the number of entities {}".format(
            self.n_entities
        )
        assert (
            np.max(tails) < self.n_entities
        ), "Tail node ID should not exceeds the number of entities {}".format(
            self.n_entities
        )
        assert (
            np.max(rels) < self.n_relations
        ), "Relation ID should not exceeds the number of relations {}".format(
            self.n_relations
        )
...
def get_dataset(
    ...
    if format_str == "built_in":
        ...
        assert (
            data_name != "FB15k"
        ), "You should provide the dataset name for raw_udd format."
        format = format_str[8:]
        dataset = KGDatasetUDDRaw(
            data_path, data_name, delimiter, files, format, has_edge_importance
        )
    elif format_str.startswith("udd"):
        # user defined dataset
        assert (
            data_name != "FB15k"
        ), "You should provide the dataset name for udd format."
        format = format_str[4:]
        dataset = KGDatasetUDD(
            data_path, data_name, delimiter, files, format, has_edge_importance
        )
    else:
