...
class ArgParser(argparse.ArgumentParser):
    def __init__(self):
        ...
        self.add_argument(
            "--topK", type=int, default=10, help="How many results are returned"
        )
        self.add_argument(
            "--score_func",
            type=str,
...
def main():
    ...
    if args.format == "h_r_t":
        if args.raw_data:
            assert emap_file is not None, (
                "When using RAW ID through --raw_data, "
                "entity_mfile should be provided."
            )
            assert rmap_file is not None, (
                "When using RAW ID through --raw_data, " "rel_mfile should be provided."
            )
            assert len(data_files) == 3, (
                "When using h_r_t, head.list, rel.list and tail.list "
                "should be provided."
            )
            ...
            head, rel, tail = load_triplet_data(
                head_f=data_files[0], rel_f=data_files[1], tail_f=data_files[2]
            )
        ...
        if args.raw_data:
            ...
            assert len(data_files) == 2, (
                "When using h_r_*, head.list and rel.list " "should be provided."
            )
            head, rel, tail, id2e_map, id2r_map = load_raw_triplet_data(
                head_f=data_files[0],
                rel_f=data_files[1],
        ...
        if args.raw_data:
            ...
            assert len(data_files) == 2, (
                "When using h_*_t, head.list and tail.list " "should be provided."
            )
            head, rel, tail, id2e_map, id2r_map = load_raw_triplet_data(
                head_f=data_files[0],
                rel_f=None,
        ...
        if args.raw_data:
            ...
            assert len(data_files) == 2, (
                "When using *_r_t rel.list and tail.list " "should be provided."
            )
            head, rel, tail, id2e_map, id2r_map = load_raw_triplet_data(
                head_f=None,
                rel_f=data_files[0],
        ...
        if args.raw_data:
            ...
            assert (
                len(data_files) == 1
            ), "When using h_*_*, only head.list should be provided."
            head, rel, tail, id2e_map, id2r_map = load_raw_triplet_data(
                head_f=data_files[0],
                rel_f=None,
                tail_f=None,
        ...
        if args.raw_data:
            ...
            assert (
                len(data_files) == 1
            ), "When using *_r_*, only rel.list should be provided."
            head, rel, tail, id2e_map, id2r_map = load_raw_triplet_data(
                head_f=None,
                rel_f=data_files[0],
                tail_f=None,
        ...
        if args.raw_data:
            ...
            assert (
                len(data_files) == 1
            ), "When using *_*_t, only tail.list should be provided."
            head, rel, tail, id2e_map, id2r_map = load_raw_triplet_data(
                head_f=None,
                rel_f=None,
                tail_f=data_files[0],
