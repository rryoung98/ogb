...
class BCELoss(BaseBCELoss):
    ...
    def __call__(self, score: th.Tensor, label):
        return -(
            label * th.log(sigmoid(score)) + (1 - label) * th.log(1 - sigmoid(score))
        )
...
class LossGenerator(BaseLossGenerator):
    def __init__(
        ...
        super(LossGenerator, self).__init__(
            neg_adversarial_sampling, adversarial_temperature, pairwise
        )
        if loss_genre == "Hinge":
            self.neg_label = -1
        ...
        if self.pairwise and loss_genre not in ["Logistic", "Hinge"]:
            raise ValueError(
                "{} loss cannot be applied to pairwise loss function".format(loss_genre)
            )
    ...
    def get_total_loss(self, pos_score, neg_score, edge_weight=None):
        ...
        if self.neg_adversarial_sampling:
            neg_loss = th.sum(
                th.softmax(neg_score * self.adversarial_temperature, dim=-1).detach()
                * neg_loss,
                dim=-1,
            )
        else:
