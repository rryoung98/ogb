...
class EmbSimInfer:
    ...
    def __init__(
        self, device, emb_file, sfunc="cosine", batch_size=DEFAULT_INFER_BATCHSIZE
    ):
    ...
    def topK(self, head=None, tail=None, bcast=False, pair_ws=False, k=10):
        ...
        if pair_ws is True:
            ...
            score = score[sidx]
            result.append(
                (F.asnumpy(head[sidx]), F.asnumpy(tail[sidx]), F.asnumpy(score))
            )
        else:
            num_head = head.shape[0]
            num_tail = tail.shape[0]
